    ...
    public class UserinfoService : IUserinfoService
    {
        ...
        public async Task<MessageResponseModel> GetByIdService(int id)
        {
            ...
            try
            {
                ...
                UserinfoModel? data = Util.ConvertDataTableToList<UserinfoModel>(res).FirstOrDefault();
                meg_res.Success = true;
                meg_res.Message = ReturnMessage.SUCCESS;
                meg_res.Code = ReturnCode.SUCCESS;
                meg_res.Data = data;
        ...
        public async Task<MessageResponseModel> SearchService(SearchUserInfoModel data)
        {
            string methodName = Util.GetMethodName();
            MessageResponseModel meg_res = new MessageResponseModel();
            meg_res.Message = ReturnMessage.SYSTEM_ERROR;
            ...
            try
            {
                ...
                //var res = Util.ConvertDataTableToList<UserinfoModel>(await _repo.Search(data));
                //var countRows = await _repo.CountData();
                //var response = new PaginatedListModel<UserinfoModel>(res, countRows, data.PageNumber, data.PageSize);

                var res = await _repo.Search(data);
                var recordData = Util.ConvertDataTableToList<UserinfoModel>(res.Item1);
                var countRows = res.Item2;
                var response = new PaginatedListModel<UserinfoModel>(recordData, (int)countRows, data.PageNumber, data.PageSize);
