...
namespace N_Health_API.Services.Master
{
    public class VehicleTypeService : IVehicleTypeService
    {
        ...
        private IVehicleTypeData _repo;

        public VehicleTypeService(IConfiguration config, IVehicleTypeData repo)
        {
            _config = config;
            _repo = repo;
        }
        public async Task<MessageResponseModel> AddService(VehicleTypeModel data, string? userCode)
        {
        ...
        public async Task<MessageResponseModel> EditService(VehicleTypeModel data, string? userCode)
        {
            string methodName = Util.GetMethodName();
            MessageResponseModel meg_res = new MessageResponseModel();
            meg_res.Message = ReturnMessage.SYSTEM_ERROR;
        ...
        public async Task<MessageResponseModel> GetByIdService(int id)
        {
            ...
            try
            {
                ...
                VehicleTypeModel? data = Util.ConvertDataTableToList<VehicleTypeModel>(res).FirstOrDefault();
                meg_res.Success = true;
                meg_res.Message = ReturnMessage.SUCCESS;
                meg_res.Code = ReturnCode.SUCCESS;
                meg_res.Data = data;
        ...
        public async Task<MessageResponseModel> SearchService(SearchVehicleTypeModel data)
        {
            string methodName = Util.GetMethodName();
            MessageResponseModel meg_res = new MessageResponseModel();
            meg_res.Message = ReturnMessage.SYSTEM_ERROR;
            ...
            try
            {
                ...
                var recordData = Util.ConvertDataTableToList<VehicleTypeModel>(res.Item1);
                var countRows = res.Item2;
                var response = new PaginatedListModel<VehicleTypeModel>(recordData, (int)countRows, data.PageNumber, data.PageSize);
