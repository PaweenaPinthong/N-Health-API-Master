...
namespace N_Health_API.Controllers
{
    ...
    public class UserinfoController : ControllerBase
    {
        ...
        [SwaggerOperation(Tags = new[] { "User" }, Summary = "ดึงข้อมูลราย Record", Description = "ดึงข้อมูลราย Record")]
        [HttpPost("GetById")]
        public async Task<MessageResponseModel> GetById([FromBody] PermissionRequest request)
        {
            ...
            try
            {
                ...
                var result = await _iUserInfo.GetByIdService(request.Permission_Id);
                msgResult.Code = result.Code;
                msgResult.Message = result.Message;
                msgResult.Success = result.Success;
                msgResult.Data = result.Data;
        ...
        [SwaggerOperation(Tags = new[] { "User" }, Summary = "ค้นหาข้อมูล", Description = "ค้นหาข้อมูลหน้าหลัก")]
        [HttpPost("Search")]
        public async Task<MessageResponseModel> Search([FromBody] SearchUserInfoModel data)
        {
            MessageResponseModel msgResult = new MessageResponseModel();
            msgResult.Code = ReturnCode.SYSTEM_ERROR;
            msgResult.Message = ReturnMessage.SYSTEM_ERROR;
            ...
            try
            {
                ...
                var result = await _iUserInfo.SearchService(data);
                msgResult.Code = result.Code;
                msgResult.Message = result.Message;
                msgResult.Success = result.Success;
                msgResult.Data = result.Data;
